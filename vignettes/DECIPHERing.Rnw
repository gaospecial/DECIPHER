...
\item Analyze sequences: find chimeras, classify into a taxonomy, predict secondary structure, create phylogenetic trees, and reconstruct ancestral states.
\end{itemize}
...
\begin{enumerate}
\item First install Command Line Tools from Apple, which contains compliers that are required to build packages on the Mac.  Next, you must install the clang complier available \underline{\url{https://cran.r-project.org/bin/macosx/tools/}{here}}.  Then, in \R{} run:
\begin{Schunk}
\begin{Sinput}
