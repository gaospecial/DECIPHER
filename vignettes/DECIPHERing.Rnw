...
\begin{enumerate}
\item Update \R{} to the latest version available from \underline{\url{http://www.r-project.org/}}.
...
\begin{enumerate}
\item First install Command Line Tools in the App Store.  Then, in \R{} run:
\begin{Schunk}
\begin{Sinput}
