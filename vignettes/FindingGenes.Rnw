...
clusts <- IdClusters(myXStringSet=intergenic, method="inexact", cutoff=0.2)
@
...
<<expr17,eval=TRUE>>=
tail(sort(table(clusts$cluster))) # the biggest clusters
AlignSeqs(intergenic[clusts$cluster==1], verbose=FALSE)
@
