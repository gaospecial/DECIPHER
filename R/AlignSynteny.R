...
#' 
#' Performs pairwise alignment of all blocks of synteny between sets of
#' sequences.
#' 
#' \code{AlignSynteny} will extract all sequence regions belonging to syntenic
#' blocks in \code{synteny}, and perform pairwise alignment with
#' \code{AlignProfiles}.  Hits are used to anchor the alignment such that only
#' the regions between anchors are aligned.
#' 
#' @name AlignSynteny
#' @param synteny An object of class ``Synteny''.
...
#' located that were used to create the object \code{synteny}.
#' @param identifier Optional character string used to narrow the search
#' results to those matching a specific identifier, or an integer sequence
#' corresponding to indices of \code{rownames(synteny)}.  If "" (the default),
#' then all identifiers are selected from \code{synteny}.
...
#' \code{gapPower}, \code{terminalGap}, \code{restrict}, \code{normPower}, and
#' \code{substitutionMatrix}.
#' @return A list with elements for each pair of \code{identifier}s in
#' \code{synteny}.  Each list element contains a \code{DNAStringSetList} one
#' pairwise alignment per syntenic block.
#' @author Erik Wright \email{eswright@@pitt.edu}
#' @seealso \code{\link{FindSynteny}}, \code{\link{Synteny-class}}
#' @examples
#' 
